...
struct BlockMMA {
  ...
  template <typename OtherMMA>
  METAL_FUNC void store_silu_gate_pair_result(
      device U* D,
      const int ldd,
      thread const OtherMMA& up_mma) const {
    D += (sm + tm) * ldd + tn + sn;
    ...
    for (short i = 0; i < TM; i++) {
      STEEL_PRAGMA_UNROLL
      for (short j = 0; j < TN; j++) {
        thread const auto& gate_accum = results[i * TN + j].thread_elements();
        thread const auto& up_accum = up_mma.results[i * TN + j].thread_elements();
        int offset = (i * TM_stride) * ldd + (j * TN_stride);

        const float gate0 = float(gate_accum[0]);
        const float gate1 = float(gate_accum[1]);
        const float up0 = float(up_accum[0]);
        const float up1 = float(up_accum[1]);
        D[offset] = U((gate0 / (1.0f + exp(-gate0))) * up0);
        D[offset + 1] = U((gate1 / (1.0f + exp(-gate1))) * up1);
      }
    }
  }
...
gemm_nt_silu_gate_pair_aligned(
    const device T* A [[buffer(0)]],
    const device T* B_gate [[buffer(1)]],
    const device T* B_up [[buffer(2)]],
    device T* D [[buffer(3)]],
    const constant GEMMParams* params [[buffer(4)]],
    uint simd_lane_id [[thread_index_in_simdgroup]],
    uint simd_group_id [[simdgroup_index_in_threadgroup]],
  ...
  B_gate += c_col_long * params->ldb;
  B_up += c_col_long * params->ldb;
  D += c_row_long * params->ldd + c_col_long;

  thread mma_t gate_mma(simd_group_id, simd_lane_id);
  thread mma_t up_mma(simd_group_id, simd_lane_id);
  thread loader_a_t loader_a(A, params->lda, As, simd_group_id, simd_lane_id);
  thread loader_b_t loader_gate(B_gate, params->ldb, Bs_gate, simd_group_id, simd_lane_id);
  thread loader_b_t loader_up(B_up, params->ldb, Bs_up, simd_group_id, simd_lane_id);
  ...
  threadgroup_barrier(mem_flags::mem_none);
  return gate_mma.store_silu_gate_pair_result(D, params->ldd, up_mma);
}
...
#define instantiate_gemm_nt_silu_gate_pair_aligned(iname, itype, oname, otype, bm, bn, bk, wm, wn) \
  template [[host_name("gemm_nt_silu_gate_pair_aligned_" #iname "_" #oname "_" #bm "_" #bn "_" #bk "_" #wm "_" #wn)]] \
  [[kernel]] void gemm_nt_silu_gate_pair_aligned<itype, bm, bn, bk, wm, wn, float>( \
      const device itype *A [[buffer(0)]], \
      const device itype *B_gate [[buffer(1)]], \
      const device itype *B_up [[buffer(2)]], \
      device itype *D [[buffer(3)]], \
      const constant GEMMParams* params [[buffer(4)]], \
      uint simd_lane_id [[thread_index_in_simdgroup]], \
      uint simd_group_id [[simdgroup_index_in_threadgroup]], \
...
instantiate_gemm_nt_silu_gate_pair_aligned(bf16, bfloat, bf16, bfloat, 32, 64, 16, 1, 2)
#endif
